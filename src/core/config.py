...
class Settings(BaseSettings):
    ...
    CAPITAL: float = Field(..., description="Capital disponível para trade")
    LEVERAGE: int = Field(..., description="Alavancagem utilizada")
    RISK_PER_TRADE: float = Field(..., description="Percentual de risco por trade (ex: 0.02 para 2%)")

    # Novos parâmetros para thresholds de entrada
    ENTRY_THRESHOLD_DEFAULT: float = Field(..., description="Threshold padrão para qualidade de entrada")

    # Novos parâmetros para risk/reward
    MIN_RR_RATIO: float = Field(..., description="Razão mínima entre take profit e stop loss")
    ATR_MULTIPLIER: float = Field(..., description="Multiplicador do ATR para stops dinâmicos")

    # Parâmetros adicionais para day trading
    VOLATILITY_HIGH_THRESHOLD: float = Field(..., description="Limite para considerar alta volatilidade (percentual)")
    VOLATILITY_LOW_THRESHOLD: float = Field(..., description="Limite para considerar baixa volatilidade (percentual)")
    MAX_POSITION_SIZE_PCT: float = Field(..., description="Tamanho máximo da posição como percentual do capital")
